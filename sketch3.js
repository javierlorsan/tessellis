...
function setup() {
    ...
    if ((tp > 3 && tp < 9) || tp == 11) {
        ...
        let rdln1 = R.random_int(0, 10);
        let rdpoly = R.random_int(5, 6);
        let rdlrpal = R.random_int(0, colores.length - 1)
        let n = R.random_int(5, 50);
        let minSize = R.random_int(1, 5);
        ...
        for (let i = 0; i < 10000; i++) {
            let siz;
            if (i > 6000) {
                siz = R.random_num(minSize, maxSize);
            } else {
                siz = map((i / 6000) ** 0.8, 0, 1, 200, minSize);
            }
            tiles.push(new Tile(R.random_num(0, sz), R.random_num(0, sz), i, alph, rdln1, rdlrpal, rdpoly, n, minSize, siz));
        }
    } else {
        makeTl();
    }
...
function makeTl() {
    ...
    for (let i = 0; i < 10000; i++) {
        ...
        drawimage(x, y, size, rdpoly, floor(R.random_num(0, 10e6)), R.random_int(0, 1));
    }

}

class Tile {
    constructor(x, y, i, alph, rdln1, rdlrpal, rdpoly, n, minsize, size) {
        this.x = x;
        this.y = y;
        this.i = i;
        this.alph = alph;
        ...
        this.maxSize = this.minSize + 5;
        this.noiseScale = 9e-11;
        this.n = n;
        this.rseed = floor(R.random_num(0, 10e6));
        this.rdcurv = R.random_int(0, 1);
        this.size = size;
        this.init();
        img.noiseSeed(floor(R.random_num(0, 10e6)));
    }
    ...
    show() {
        ...
        img.strokeWeight(this.size);

        if (tp < 2 && strk > 0.5) { img.noStroke(); }

        if (floor(this.x / width * this.n * 2) % 2 == 0) {
            if ((tp < 3) && strk > 0.5) {
                img.fill(lerpColorScheme(curlNoise(this.x * this.noiseScale, (this.y + 0) * this.noiseScale, 0), colores[this.rdlrpal], this.alph));
                if (tp == 2) { img.strokeWeight(1);}
            } else {
                img.stroke((lerpColorScheme(curlNoise(this.x * this.noiseScale, (this.y + 0) * this.noiseScale, 0), colores[this.rdlrpal], this.alph)));
                img.strokeWeight(this.size);
            }
        } else {
            if ((tp < 3) && strk >= 0.5) {
                img.fill(lerpColorScheme(curlNoise(this.x * this.noiseScale, (this.y + 0) * this.noiseScale, 0), palette, this.alph));
                if (tp == 2) { img.stroke("#000000"); }
            }
            else {
                img.stroke((lerpColorScheme(curlNoise(this.x * this.noiseScale, (this.y + 0) * this.noiseScale, 0), palette, this.alph)))
                if (this.rdln1 == 1 && (tp > 2 && tp != 9 && tp != 10)) img.strokeWeight(1);

            }
        }
       
        drawimage(this.x, this.y, this.size, this.rdpoly, this.rseed, this.rdcurv);
       
    }

    move() {
        this.noiseScale += 0.000000000001;
    }

    init() {
        this.noiseScale = 9e-11;
        /*if (this.i > 6000) {
            this.size = R.random_num(this.minSize, this.maxSize);
        } else {
            this.size = map((this.i / 6000) ** 0.8, 0, 1, 200, this.minSize);
        }*/
    }
}
...
function writeArcs() {
    ...
    for (let a of arcs) {
        if (a.id == id) {
            ...
            if (r <= 155 && b <= 155 && g <= 155) { img1.stroke(cline1); } else { img1.stroke(cline2); }
            img1.arc(a.x, a.y, a.d, a.d, a.theta1, a.theta2);
        }
    }
